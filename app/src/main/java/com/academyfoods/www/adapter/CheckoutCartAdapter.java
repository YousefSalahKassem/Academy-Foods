...
public class CheckoutCartAdapter extends RecyclerView.Adapter<CheckoutCartAdapter.mh> {
    ArrayList<PopularModel>list;
    Context context;

    public CheckoutCartAdapter(ArrayList<PopularModel> list, Context context) {
        this.list = list;
    ...
    @NonNull
    @Override
    public CheckoutCartAdapter.mh onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.checkout_cart,parent,false);
        return new mh(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CheckoutCartAdapter.mh holder, int position) {
